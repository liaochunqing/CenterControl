...
#import "APRenameGroupView.h"
#import "AppDelegate.h"

@implementation APRenameGroupView
...
//        make.centerY.mas_equalTo(self);
    ...
    im.image = [UIImage imageNamed:@"Group 11674"];
    [_baseview addSubview:im];
    [im mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(namelab.mas_bottom).offset(2*Left_Gap);
        make.centerX.mas_equalTo(_baseview);
        make.size.mas_equalTo(CGSizeMake(W_SCALE(78), H_SCALE(78)));
    }];
...
//        make.centerY.mas_equalTo(_baseview);
        make.top.mas_equalTo(im.mas_bottom).offset(Left_Gap);
...
-(void)writeDB
{
    ...
    if ([db open])
    {
        NSString *sqlStr = [NSString stringWithFormat:@"UPDATE zk_group set group_name='%@' where id=='%@'",_deviceInfo.name,_deviceInfo.nodeId];
