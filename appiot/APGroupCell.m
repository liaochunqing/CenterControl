...
-(void)updateCellWithData:(APGroupNote*)node index:(int)row
{
    ...
    CGFloat midGap = W_SCALE(36)+Left_Gap;//cell各个图标文字中间的间隙
    ...
    [btn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(W_SCALE(103), Group_Cell_Height));
    }];
...
//    expendW = _expendBtn.hidden?0:W_SCALE(30);
    CGFloat imX = Left_Gap + expendW + Left_Gap + midGap* (node.depth);
    ...
    [_im mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(imW, imH));
    }];
    
    //标题
    _title = [[UILabel alloc] init];
    _title.textColor = node.isDevice ? ColorHex(0xABBDD5):ColorHex(0xFFFFFF);;
    ...
    [_title mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(self.contentView);
        make.left.mas_equalTo(_im.mas_right).offset(Left_Gap);
